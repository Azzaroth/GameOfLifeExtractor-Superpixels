...
#define atEdgeMap(x,y) edgeMap[(x)+(y)*width]
  ...
  edgeMap = calloc(numPixels, sizeof(int)) ;
  masses = malloc(sizeof(vl_uint32) * numPixels) ;
  ...
  {
    vl_uint32 * cleaned = calloc(numPixels, sizeof(vl_uint32)) ;
    vl_uindex * segment = malloc(sizeof(vl_uindex) * numPixels) ;
    vl_size segmentSize ;
    vl_uint32 label ;
